...
def test_board_from_string() -> None:
    assert schemas.Board.from_string(STRING_BOARD) == BOARD


def test_board_as_string() -> None:
    assert BOARD.as_string() == STRING_BOARD


def test_solve_board() -> None:
    new_board, success = service.solve_board(BOARD)
    assert success
    assert new_board == SOLUTION


def test_solve_board_fails_on_unsolvable_board() -> None:
    _, success = service.solve_board(UNSOLVABLE_BOARD)
    assert success is False


def test_solve_board_endpoint(client: starlette.testclient.TestClient) -> None:
    response = client.get("/solve", params={"board": STRING_BOARD})
    assert response.status_code == 200
    assert response.json() == STRING_SOLUTION


def test_solve_board_endpoint_fails_on_unsolvable_board(
    client: starlette.testclient.TestClient,
) -> None:
    response = client.get("/solve", params={"board": UNSOLVABLE_STRING_BOARD})
